...
1
#3 - Add a new card
def add_card(name, mana, power, colors, type, text, legalities):
    collection.update_one({},{"$set":{"cards." + name:{"name": name, "convertedManaCost": float(mana), "power":power, "colors": colors, "type": type, "text": text, "legalities": legalities}}})
...
#9 - Check card legalities in a certain format
def check_legalities(name, format):
    cards = collection.aggregate([
        {"$project":
            {"cards":
                {"$map":
    ...
    try:
        cards.next()
        print("| " + name + " is legal in " + format + " format")
    except:
        print("| There is no card named " + name + " in " + format + " format")
...
while choice != 0:
    ...
    if choice == "0": #0 - Exit
        ...
        if is_loaded == False:
            ...
            print("| Insert the legalities (separated by a comma ','):")
            raw_legalities = input("| >> ")
            raw_legalities = raw_legalities.split(",")
            
            legalities = {}
            for i in raw_legalities:
                legalities[i] = "Legal"

            add_card(name, mana, power, colors, type, text, legalities)
    ...
    elif choice == "9": #9 - Check card legalities in a certain format
        clear_terminal()
        ...
        if is_loaded == False:
            ...
            print("| Insert the format:")
            format = input("| >> ")

            check_legalities(name, format)

    elif choice == "10": #10 - Drop database
        clear_terminal()

        if is_loaded == False:
            print("\n| No database found. Nothing to drop")
        else:
            drop_db()
            is_loaded = False
            print("\n| database dropped")
